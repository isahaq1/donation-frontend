...
const SidebarItem = ({ item, pageName, setPageName }: any) => {
  ...
  const filteredSubMenu = item.children?.filter((child: any) => {
    if (isAdmin == "true") {
      return true; // Admin can see all submenu items
    }
    if (child.permission == "user") {
      return true;
    }
    return false;
    // Regular user can see items they have permission for
  });

  const showSubMenu =
    isAdmin == "true" || (filteredSubMenu && filteredSubMenu.length > 0);
